...
class Engineer:
    ...
    def _acquire_device(self):
        if self.args.use_gpu:
            # Ensure self.args.gpu is a list
            if isinstance(self.args.gpu, int):
                self.args.gpu = [self.args.gpu]

            os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(map(str, self.args.gpu))  # Ensure all GPUs are visible
    ...
    def _build_model(self):
        args = self.args
        self.model = Cnns(tuple(args.in_shape), args.hid_S,
                           args.hid_T, args.N_S, args.N_T).to(self.device)
        if self.args.use_gpu and torch.cuda.device_count() > 1:
            gpu_ids = [0, 1]
